...
> [!IMPORTANT]
> The requirement to get credentials for the TCPX kernel is specific to the 
> A3 High machine types, which rely on GPUDirect-TCPX for their networking.
> G4 instances do not use GPUDirect-TCPX ( source). Instead, they rely on 
> Google Titanium offloads for dedicated network processing up to 400 Gbps and 
> use direct PCIe Peer-to-Peer (P2P) communication to accelerate multi-GPU workloads.
...
Set the values in `g4-standard-slurm-deployment.yaml` for your deployment
...
Set the deployment variable `g4_reservation_name` to the reservation name provided by Google.

```yaml
  # g4_reservation_name must be specified; if Google staff have provided you
  # with a reservation name, use it. Otherwise supply user-created reservation.
  g4_reservation_name: reservation-name-provided-by-google
```
...
```shell
gcloud compute reservations create ${G4_RESERVATION_NAME} \
    --project=${PROJECT_ID} \
    --machine-type=g4-highgpu-8g \
    --vm-count=${N_VMS} \
    --zone=${ZONE} \
...
(e.g. g4-standard-96 VM in configured zone). Ensure that the reservation name in the
...
In order to make use of DWS Flex Start mode with SlurmGCP, set the `g4_dws_flex_enabled` variable as shown below

```yaml
  vars: 
    g4_dws_flex_enabled: true             # enabling dws flex by setting the variable to true
...
Similarly to make use of Spot VMs with Slurm, set the `g4_enable_spot_vm` variable as shown below

```yaml
  vars: 
    g4_enable_spot_vm: true             # enabling spot vm by setting the variable to true
    # the rest of the variables
```
...
size using `g4_static_cluster_size` variable. Recall that there is 1 NVIDIA RTX-PRO-6000 GPU per g4-standard-96 VM.

```yaml
  g4_static_cluster_size: 32
```
...
```shell
./gcluster deploy -d examples/machine-learning/g4-standard-96/g4-standard-slurm-deployment.yaml examples/machine-learning/g4-standard-96/g4-standard-slurm-blueprint.yaml --auto-approve
```
